...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                imprimirMensajeRecibido(inputLine,clientSocket.getInetAddress().getHostAddress(),clientSocket.getPort());
                if (hayQueCerrar(inputLine)) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            imprimirConexionCerrada(clientSocket);
        } catch (IOException e) {
            imprimirErrorComunicacion(e);
        } finally {
            try {
                clientSocket.close();
            } catch (IOException e) {
                imprimirErrorCierre(e);
            }
        }
    }
    ...
    private static void imprimirErrorCierre(IOException e) {
        System.err.println("Error closing socket: " + e.getMessage());
    }

    private static void imprimirErrorComunicacion(IOException e) {
        System.err.println("Problem with communication with client: " + e.getMessage());
    }

    protected static void imprimirConexionCerrada(Socket clientSocket) {
        System.out.println("Connection closed with " + clientSocket.getInetAddress().getHostAddress() + ":"
                + clientSocket.getPort());
    }

    protected static void imprimirMensajeRecibido(String inputLine, String hostAddress, int port) {
